...
// Steps for dept flow: Hospital → Doctor & Time → Your Details → Review
const STEPS = [
...
const generateSlots = (doctor, date) => {
  ...
  const avail = doctor.availability?.find(a => a.day?.toLowerCase() === dayName.toLowerCase())
  const toMins = str => { if (!str) return null; const [h,m] = str.split(':').map(Number); return h*60+m }
  const toTime = mins => { let h=Math.floor(mins/60); const m=mins%60, ap=h>=12?'PM':'AM'; h=h%12||12; return `${h}:${String(m).padStart(2,'0')} ${ap}` }
  if (avail?.startTime && avail?.endTime) {
    const start=toMins(avail.startTime), end=toMins(avail.endTime)
  ...
  return ['9:00 AM','9:15 AM','9:30 AM','9:45 AM','10:00 AM','10:15 AM','10:30 AM','10:45 AM',
          '11:00 AM','11:15 AM','11:30 AM','11:45 AM','2:00 PM','2:15 PM','2:30 PM','2:45 PM',
          '3:00 PM','3:15 PM','3:30 PM','3:45 PM','4:00 PM','4:15 PM','4:30 PM','4:45 PM']
}

// ─── MAIN COMPONENT ───────────────────────────────────────────────────────────
export default function DeptBookingFlow({ onHome, dept }) {
  const { user: patient } = useAuth()
  const [showAuth, setShowAuth] = useState(false)
  const [step, setStep] = useState(1)

  const [sel, setSel] = useState({
    hospital: null, doctor: null, time: null, date: today,
    name: '', phone: '', age: '', gender: '', notes: '',
  })
  ...
  const fetchHospitals = async () => {
    ...
    try { setAllHospitals(await getHospitals()) }
  ...
  useEffect(() => {
    if (patient) {
      setSel(s => ({
        ...
        phone:  s.phone  || patient.phone  || '',
        age:    s.age    || (patient.age ? String(patient.age) : ''),
        gender: s.gender || patient.gender || '',
      }))
    }
  }, [patient])
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try { setBookedSlots(await getBookedSlots(sel.doctor._id, sel.date)) }
  ...
  // ── DERIVED: hospitals that have doctors in this dept ─────────────────────
  const hospitalsForDept = useMemo(() => {
    if (!dept || allDoctors.length === 0 || allHospitals.length === 0) return allHospitals
    const deptName = dept.departmentName?.toLowerCase()
    const vendorIds = new Set(
      allDoctors
        .filter(doc => doc.specialization?.toLowerCase() === deptName)
        .map(doc => String(doc.vendorId?._id ?? doc.vendorId))
        .filter(Boolean)
    )
    const filtered = allHospitals.filter(h => vendorIds.has(String(h._id)))
    return filtered.length > 0 ? filtered : allHospitals
  }, [allDoctors, allHospitals, dept])

  // ── DERIVED: doctors for selected hospital + this dept ────────────────────
  const doctorsForSlot = useMemo(() => {
    if (!dept) return []
    const deptName = dept.departmentName?.toLowerCase()
    return allDoctors.filter(doc => {
      const sameDept = doc.specialization?.toLowerCase() === deptName
      if (!sel.hospital) return sameDept
      const sameVendor = String(doc.vendorId?._id ?? doc.vendorId) === String(sel.hospital._id)
      return sameDept && sameVendor
    })
  }, [allDoctors, dept, sel.hospital])
  ...
  const canContinue = () => {
    ...
    if (step === 2) return !!sel.doctor && !!sel.time
    if (step === 3) return sel.name.trim() && sel.phone.trim() && sel.age.trim() && sel.gender.trim()
    return true
  }

  const handleBack = () => {
    if (step === 1) { onHome(); return }
    if (step === 2) setSel(s => ({ ...s, doctor: null, time: null }))
    setStep(s => s - 1)
  }
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (res._id || res.success || appt._id) {
        setTokenNum(appt.tokenNumber ?? res.tokenNumber ?? '—')
        setBookingId(appt._id ?? res._id ?? null)
        setConfirmed(true)
      } else {
        ...
        <div className={styles.confirmed}>
          ...
          <div className={styles.tokenCard}>
            ...
            <div className={styles.tokenMeta}>
              ...
              <span>{dept?.departmentName}</span><span>·</span>
        ...
        <div className={styles.layout}>
          ...
          <div className={styles.sidebar}>
            ...
            <div className={styles.tip} style={{ flexDirection:'column', gap:7 }}>
              <strong style={{ fontSize:11, color:'#374151', letterSpacing:'0.05em', textTransform:'uppercase' }}>Selected Department</strong>
              <div style={{ display:'flex', alignItems:'center', gap:8, fontSize:14, fontWeight:600 }}>
                {deptIcon(dept?.departmentName)} {dept?.departmentName}
              </div>
            ...
            <div className={styles.tip}>
              <span>💡</span>
              <div><strong>Tip</strong><p>Arrive 10 minutes before your token is called.</p></div>
            </div>
          </div>
            ...
            {step === 1 && (
              <div className={styles.stepContent}>
                <div className={styles.stepHeader}>
                  <div className={styles.stepIcon}>{deptIcon(dept?.departmentName)}</div>
                  <div>
                    <h2>Choose a Hospital</h2>
                    <p>Hospitals offering <strong>{dept?.departmentName}</strong></p>
                  </div>
                </div>

                {loadingH && <Spinner />}
                {loadingDoc && !loadingH && <Spinner />}
                {errH && <ErrBox msg={errH} onRetry={fetchHospitals} />}

                {!loadingH && !loadingDoc && !errH && hospitalsForDept.length === 0 && (
                  <div className={styles.emptyBox}>No hospitals found for this department.</div>
                )}

                {!loadingH && !loadingDoc && !errH && hospitalsForDept.length > 0 && (
                  <div className={styles.hospitalGrid}>
                    {hospitalsForDept.map(h => (
                      <button key={h._id}
                        className={`${styles.hospitalCard} ${sel.hospital?._id===h._id?styles.hospitalSelected:''}`}
                        onClick={() => setSel(s => ({ ...s, hospital:h, doctor:null, time:null }))}>
                        {sel.hospital?._id === h._id && <div className={styles.hospitalCheck}>✓</div>}
                        <div className={styles.hospitalIcon}>🏥</div>
                        <div className={styles.hospitalName}>{h.organisationName}</div>
                        {h.address && <div className={styles.hospitalInfo}>📍 {h.address}</div>}
                        {h.phone   && <div className={styles.hospitalInfo}>📞 {h.phone}</div>}
                      </button>
                    ))}
                  </div>
                )}
              ...
              <div className={styles.stepContent}>
                <div className={styles.stepHeader}>
                  ...
                  <div>
                    ...
                    <p>{dept?.departmentName} doctors at {sel.hospital?.organisationName}</p>
                  </div>
                </div>
                ...
                {!loadingDoc && !errDoc && doctorsForSlot.length === 0 && (
                  <div className={styles.emptyBox}>No doctors available for {dept?.departmentName} at this hospital.</div>
                )}
                ...
                {!loadingDoc && !errDoc && doctorsForSlot.length > 0 && (
                  <div className={styles.doctorGrid}>
                    {doctorsForSlot.map(doc => (
                      <button key={doc._id}
                        className={`${styles.doctorCard} ${sel.doctor?._id===doc._id?styles.doctorSelected:''}`}
                        onClick={() => setSel(s => ({ ...s, doctor:doc, time:null }))}>
                        {sel.doctor?._id === doc._id && <div className={styles.docCheckBadge}>✓</div>}
                        <img src={doc.photo} alt={doc.name} className={styles.docPhoto}
                          onError={e => { e.target.src="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='%23aaa'%3E%3Cpath d='M12 12c2.7 0 4.8-2.1 4.8-4.8S14.7 2.4 12 2.4 7.2 4.5 7.2 7.2 9.3 12 12 12zm0 2.4c-3.2 0-9.6 1.6-9.6 4.8v2.4h19.2v-2.4c0-3.2-6.4-4.8-9.6-4.8z'/%3E%3C/svg%3E" }} />
                        <div className={styles.docName}>{doc.name}</div>
                        <div className={styles.docSpec}>{doc.specialization}</div>
                        <div className={styles.docClinic}>🏥 {doc.vendorId?.organisationName||'N/A'}</div>
                        <div className={styles.docClinic}>
                          📅 {doc.availability?.length>0 ? doc.availability.map(a=>a.day.slice(0,3)).join(', ') : 'N/A'}
                        </div>
                      </button>
                    ))}
                  </div>
                ...
                {sel.doctor && (
                  <>
                    <h3 className={styles.subLabel} style={{marginTop:28}}>Select Date</h3>
                    <input type="date" min={today} value={sel.date}
                      onChange={e => setSel(s => ({ ...s, date:e.target.value, time:null }))}
                      style={{ fontFamily:'DM Sans, sans-serif', fontSize:'0.92rem', border:'1.5px solid var(--border)', borderRadius:10, padding:'10px 14px', outline:'none', background:'var(--bg)', color:'var(--text)', cursor:'pointer', maxWidth:240, marginBottom:20, display:'block' }} />

                    <h3 className={styles.subLabel}>Available Slots</h3>
                    {loadingSlots ? <Spinner /> : timeSlots.length > 0 ? (
                      <div className={styles.timeGrid}>
                        {timeSlots.map((t,i) => {
                          const isBooked = bookedSlots.includes(t)
                          const now = new Date()
                          const isToday = new Date(sel.date+'T00:00:00').toDateString() === now.toDateString()
                          const toMin = time => { const [tm,period]=time.split(' '); let [h,m]=tm.split(':').map(Number); if(period==='PM'&&h!==12)h+=12; if(period==='AM'&&h===12)h=0; return h*60+m }
                          const isPast = isToday && toMin(t) < now.getHours()*60+now.getMinutes()
                          const isDisabled = isBooked || isPast
                          return (
                            <button key={i} disabled={isDisabled}
                              className={`${styles.timeSlot} ${sel.time===t?styles.timeSelected:''} ${isDisabled?styles.timeBooked:''}`}
                              onClick={() => !isDisabled && setSel(s => ({ ...s, time:t }))}>
                              {t} {isBooked?'🔴':isPast?'⏰':''}
                            </button>
                          )
                        })}
                      ...
                      <div className={styles.emptyBox}>No slots available for this date.</div>
                    )}
                  </>
                )}
              </div>
              ...
              <div className={styles.stepContent}>
                ...
                <div className={styles.review}>
                  <div className={styles.reviewRow}>
                    <span style={{display:'flex',alignItems:'center',gap:8}}><LocalHospitalIcon style={{fontSize:18,color:'#0369a1'}}/> Hospital</span>
                    <strong>{sel.hospital?.organisationName}</strong>
                  </div>
                  {sel.hospital?.address && (
                    <div className={styles.reviewRow}>
                      <span style={{display:'flex',alignItems:'center',gap:8}}><LocationOnIcon style={{fontSize:18,color:'#15803d'}}/> Location</span>
                      <strong>{sel.hospital.address}</strong>
                    </div>
                  )}
                  <div className={styles.reviewRow}>
                    <span style={{display:'flex',alignItems:'center',gap:8}}><MedicalServicesIcon style={{fontSize:18,color:'#7e22ce'}}/> Department</span>
                    <strong>{dept?.departmentName}</strong>
                  </div>
                  <div className={styles.reviewRow}>
                    <span style={{display:'flex',alignItems:'center',gap:8}}><PersonIcon style={{fontSize:18,color:'#c2410c'}}/> Doctor</span>
                    <strong>{sel.doctor?.name}</strong>
                  </div>
                  <div className={styles.reviewRow}>
                    <span style={{display:'flex',alignItems:'center',gap:8}}><CalendarMonthIcon style={{fontSize:18,color:'#15803d'}}/> Date</span>
                    <strong>{new Date(sel.date+'T12:00:00').toLocaleDateString('en-IN',{weekday:'short',day:'numeric',month:'long',year:'numeric'})}</strong>
                  </div>
                  <div className={styles.reviewRow}>
                    <span style={{display:'flex',alignItems:'center',gap:8}}><AccessTimeIcon style={{fontSize:18,color:'#1d4ed8'}}/> Time Slot</span>
                    <strong>{sel.time}</strong>
                  </div>
                  <div className={styles.reviewRow}>
                    <span style={{display:'flex',alignItems:'center',gap:8}}><AccountCircleIcon style={{fontSize:18,color:'#9d174d'}}/> Patient</span>
                    <strong>{sel.name}</strong>
                  </div>
                  <div className={styles.reviewRow}>
                    <span style={{display:'flex',alignItems:'center',gap:8}}><PhoneIcon style={{fontSize:18,color:'#15803d'}}/> Phone</span>
                    <strong>{sel.phone}</strong>
                  </div>
                  <div className={styles.reviewRow}>
                    <span style={{display:'flex',alignItems:'center',gap:8}}><CakeIcon style={{fontSize:18,color:'#c2410c'}}/> Age / Gender</span>
                    <strong>{sel.age} yrs · {sel.gender}</strong>
                  </div>
                  {sel.notes && (
                    <div className={styles.reviewRow}>
                      <span style={{display:'flex',alignItems:'center',gap:8}}><NoteAltIcon style={{fontSize:18,color:'#374151'}}/> Notes</span>
